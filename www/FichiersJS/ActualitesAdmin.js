...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("statusFilter").addEventListener("change", loadNews);
    ...
    // Preview de l'image lors du changement
    document
    ...
    document
      .getElementById("add-hashtag-button")
      .addEventListener("click", addHashtag);
    document
      .getElementById("hashtag-input")
...
// Fonction pour ajouter un hashtag
// function addHashtag() {
...
// Fonction pour supprimer un hashtag
...
// Fonction pour afficher les hashtags
...
// Afficher la modale de connexion (ne devrait plus être utilisé avec la nouvelle vérification)
...
//     } else {
//       showAlert(
//         data.message || "Erreur lors du chargement des actualités",
//         "error"
//       );
//     }
//   } catch (error) {
...
function renderNews(newsList) {
  ...
  if (!Array.isArray(newsList)) {
    console.error(
      "La liste des actualités n'est pas un tableau:",
      newsList
    );
    newsList = [];
  }
  ...
  if (newsList.length === 0) {
    ...
    row.innerHTML =
      '<td colspan="8" style="text-align: center;">Aucune actualité trouvée</td>';
  ...
  newsList.forEach((news) => {
    ...
    row.innerHTML = `
      ...
      <td class="description-cell" title="${news.description}">${
      news.description
    }</td>
      ...
      <td>${createdDate}</td>
      <td>${
        news.mediaUrl || "<span>Aucune URL</span>"
      }</td> <!-- Colonne pour le mediaUrl -->
      <td>
...
// Afficher la modale d'ajout/modification d'actualité
...
//       // Gestion des hashtags
//       currentHashtags = Array.isArray(news.hashtags) ? [...news.hashtags] : [];
//       renderHashtags();
//       document.getElementById("hashtags").value = JSON.stringify(currentHashtags);
...
// Gérer la soumission du formulaire d'actualité
...
//   // Vérifiez les hashtags avant de les ajouter au FormData
...
async function handleNewsSubmit(e) {
  ...
  try {
    ...
    if (response.ok) {
      showAlert(
        `Actualité ${isEditing ? "modifiée" : "ajoutée"} avec succès`,
        "success"
      );
      closeNewsModal();
      loadNews();
      ...
      showAlert(
        error.message ||
          `Erreur lors de ${isEditing ? "la modification" : "l'ajout"} de l'actualité`,
        "error"
      );
    }
  } catch (error) {
